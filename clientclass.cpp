...
int send2socket(char* arg_host, int arg_port, char* msg)
{

    meusocket.soconnect(arg_host,arg_port);
    //connect_socket(arg_host,arg_port);
    meusocket.sowrite(msg);
    //send_msg(msg);

    //receive_msg();
    sleep(1);
    printf("%s\n",meusocket.soread());                          // printa resposta
